...
export default function GroupTable({title, types}: {title: string, types: Types[]}) {
  ...
  const plannedTotal: number = types?.reduce((acc, types) => acc + types.planned, 0)
  const spentTotal: number = types?.reduce((acc, types) => acc + types.spent, 0)
